...
class PriceConverter {
  static String convertPrice(BuildContext context, double price, {double discount, String discountType}) {
    ...
    bool _inRight = Provider.of<SplashProvider>(context, listen: false).configModel.currencySymbolPosition == 'left';
  ...
  static String convertPriceOne(BuildContext context, double price, double unitNumbers) {
    return '${(price * Provider.of<SplashProvider>(context, listen: false).myCurrency.exchangeRate
        * (1/Provider.of<SplashProvider>(context, listen: false).usdCurrency.exchangeRate)/ unitNumbers).toStringAsFixed(Provider.of<SplashProvider>(context,listen: false).configModel.decimalPointSetting??1).replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')
        }';
  }
}