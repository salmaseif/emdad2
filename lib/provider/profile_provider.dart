...
class ProfileProvider extends ChangeNotifier {
  ...
  Future<String> getUserInfo(BuildContext context) async {
    ...
    if (apiResponse.response != null && apiResponse.response.statusCode == 200) {
      ...
      print('userInfoModel::::::::: ${_userInfoModel.isComplete}');
      print('userInfoModel::::::::: ${_userInfoModel.isDoc}');
    } else {
      ApiChecker.checkApi(context, apiResponse);
    }
    notifyListeners();
  ...
  Future<ResponseModel> updateUserDocInfo(UserInfoModel updateUserModel, File file, String token) async {
    _isLoading = true;
    notifyListeners();
    ResponseModel responseModel;
    http.StreamedResponse response = await profileRepo.updateProfileDoc(updateUserModel, file, token);
    if (response.statusCode == 200) {
      Map map = jsonDecode(await response.stream.bytesToString());
      String message = map["message"];
      _userInfoModel = updateUserModel;
  ...
  Future<ResponseModel> updateCompanyDocInfo(UserInfoModel updateUserModel, pass, String token) async {
    _isLoading = true;
    notifyListeners();
    print("updateUserModel: ${updateUserModel.saleAmount}");

    ResponseModel responseModel;
    http.StreamedResponse response = await profileRepo.updateCompanyProfile(updateUserModel, pass, token);
    _isLoading = false;
    if (response.statusCode == 200) {
      Map map = jsonDecode(await response.stream.bytesToString());
      String message = map["message"];
