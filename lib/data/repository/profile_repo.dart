...
class ProfileRepo {
  ...
  Future<http.StreamedResponse> updateProfileDoc(UserInfoModel userInfoModel, File file, String token) async {
    http.MultipartRequest request = http.MultipartRequest('POST', Uri.parse('${AppConstants.BASE_URL}${AppConstants.UPDATE_PROFILE_DOC_URI}'));
    request.headers.addAll(<String,String>{'Authorization': 'Bearer $token'});
    Map<String, String> _fields = Map();
    if(file != null){
      request.files.add(http.MultipartFile('imageDoc1', file.readAsBytes().asStream(), file.lengthSync(), filename: file.path.split('/').last));
    }
    _fields.addAll(<String, String>{
      '_method': 'put', 'f_name': userInfoModel.fName, 'l_name': userInfoModel.fName, 'phone': userInfoModel.phone
    });
    request.fields.addAll(_fields);
    http.StreamedResponse response = await request.send();
  ...
  Future<http.StreamedResponse> updateCompanyInfo(UserInfoModel userInfoModel, pass, String token) async {
    http.MultipartRequest request = http.MultipartRequest('POST', Uri.parse('${AppConstants.BASE_URL}${AppConstants.UPDATE_PROFILE_DOC_URI}'));
    request.headers.addAll(<String,String>{'Authorization': 'Bearer $token'});
    Map<String, String> _fields = Map();
    request.fields.addAll(_fields);
  ...
  Future<http.StreamedResponse> updateCompanyProfile(UserInfoModel userInfoModel, String pass, String token) async {
    http.MultipartRequest request = http.MultipartRequest('POST', Uri.parse('${AppConstants.BASE_URL}${AppConstants.UPDATE_PROFILE_URI}'));
    request.headers.addAll(<String,String>{'Authorization': 'Bearer $token'});
    Map<String, String> _fields = Map();
    if(pass.isEmpty) {
      _fields.addAll(<String, String>{
        '_method': 'put', 'f_name': userInfoModel.fName, 'l_name': userInfoModel.lName, 'phone': userInfoModel.phone, 'treadType': userInfoModel.treadType, 'categorySelected':  userInfoModel.categorySelected, 'saleAmount':  userInfoModel.saleAmount
      });
    }else {
      _fields.addAll(<String, String>{
        '_method': 'put', 'f_name': userInfoModel.fName, 'l_name': userInfoModel.lName, 'phone': userInfoModel.phone, 'password': pass, 'treadType': userInfoModel.treadType, 'categorySelected':  userInfoModel.categorySelected, 'saleAmount':  userInfoModel.saleAmount
      });
    }
    request.fields.addAll(_fields);
    print('========>${_fields.toString()}');
